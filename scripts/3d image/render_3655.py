...
out=Path('results/3655')
ROWS=int(sys.argv[1]) if len(sys.argv)>1 else 80
...
assert len(data['placements'])==len(data['parts'])==33
...
d.text((L,25),'神奈川冲浪里 · 全零件拼豆图纸',font=titlefont,fill='#111111')
text(L,46,'神奈川冲浪里 · 全零件拼豆图纸',34,anchor='start')
desc=f'52 × {ROWS} 格  |  33 个零件  |  2,504 颗豆  |  色号依原图'
...
svg.append('</svg>')
(out/f'great-wave-52x{ROWS}.svg').write_text('\n'.join(svg),encoding='utf-8')
im.save(out/f'great-wave-52x{ROWS}.png',dpi=(300,300))
im.resize((W//2,H//2)).save(out/'preview.png')
with open(out/f'great-wave-52x{ROWS}.csv','w',newline='',encoding='utf-8-sig') as f:
 wr=csv.writer(f);wr.writerows([[codes[v] if v>=0 else '' for v in row] for row in grid])
json.dump({'columns':52,'rows':ROWS,'parts':33,'beads':2504,'counts':{code:int((grid==i).sum()) for i,code in enumerate(codes)},'checks':['all original grids unchanged','no rotation','no overlap','at least one empty cell between different parts']},open(out/'verification.json','w'),ensure_ascii=False,indent=2)
print(f'Verified 33 parts, 2504 cells, 52 x {ROWS}, no overlap, one-cell clearance.')
