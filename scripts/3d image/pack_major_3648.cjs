...
const ps=input.parts.slice(0,7),n=ps.length,w=ps.map(p=>p.w),h=ps.map(p=>p.h);
...
let init=JSON.parse(fs.readFileSync('results/3648/best_partial.json')).parts;let best=Infinity,bx=init.slice(0,7).map(p=>p.x),by=init.slice(0,7).map(p=>p.y),start=Date.now();
const ri=n=>Math.floor(Math.random()*n);
for(let run=0;run<300;run++){
 let xs=ps.map(p=>ri(53-p.w)),ys=ps.map(p=>ri(79-p.h));
 if(run%3&&bx){xs=bx.slice();ys=by.slice();}
 let total=0;
 for(let i=0;i<n;i++)for(let j=i+1;j<n;j++)total+=cost(i,j,xs[i],ys[i],xs[j],ys[j]);
 for(let step=0;step<500000;step++){
  const temp=Math.max(.1,(run%3===0?500:run%3===1?35:8)*Math.pow(1-step/500000,3));
  let i=ri(n),old=0;
  ...
  if(total===0){data.placements=ps.map((p,i)=>({...p,x:xs[i],y:ys[i]}));fs.writeFileSync('results/3648/major_layout.json',JSON.stringify(data));console.log('SUCCESS',run,step,(Date.now()-start)/1000);process.exit();}
 }
 console.log(run,best,(Date.now()-start)/1000);
 fs.writeFileSync('results/3648/major_partial.json',JSON.stringify({best,parts:ps.map((p,i)=>({...p,x:bx[i],y:by[i]}))}));
}
