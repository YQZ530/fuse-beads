...
let init=JSON.parse(fs.readFileSync('results/3648/seed.json')).parts;let best=Infinity,bx=init.slice(0,7).map(p=>p.x),by=init.slice(0,7).map(p=>p.y),start=Date.now();
const ri=n=>Math.floor(Math.random()*n);
for(let run=0;run<300;run++){
 let xs=ps.map(p=>ri(53-p.w)),ys=ps.map(p=>ri(79-p.h));
 if(bx){xs=bx.slice();ys=by.slice();}
 let total=0;
 for(let i=0;i<n;i++)for(let j=i+1;j<n;j++)total+=cost(i,j,xs[i],ys[i],xs[j],ys[j]);
 for(let step=0;step<500000;step++){
  const temp=Math.max(.1,(run%3===0?20:run%3===1?35:8)*Math.pow(1-step/500000,3));
  let i=ri(n),old=0;
