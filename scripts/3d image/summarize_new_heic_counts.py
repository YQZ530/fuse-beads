...
assert len(counts)==200 and sum(counts.values())==124278
header='200 个色号，共 124,278 颗豆；与第一页总数一致。\n跨页重复项仅计一次，66 项重复数量全部一致。\n仅列出原图出现的色号；未出现的色号不补零。\n'
...
(out/'counts-A1-ZG.json').write_text(json.dumps({'counts':counts,'observations':observations,'verification':{'unique_colors':200,'total_beads':124278,'header_matches':True,'duplicate_observations':66,'conflicts':[]},'method':'Visual transcription of circle labels and counts below; layout approach reviewed against stage2_analyze_helper_modal_legend.py; exact-code deduplication.'},ensure_ascii=False,indent=2),encoding='utf-8')
print('\n'.join(lines))
