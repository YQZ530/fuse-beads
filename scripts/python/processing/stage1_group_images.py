...
DEFAULT_MANIFEST_PATH = Path("results") / "processing" / "1.grouped-images" / "groups.manifest.json"
CACHE_VERSION = 5
...
def main() -> int:
    ...
    parser.add_argument("--manifest", default="", help="Manifest JSON path. Defaults to results/processing/1.grouped-images/groups.manifest.json.")
    parser.add_argument("--no-paddle", action="store_true", help="Disable PaddleOCR fallback when Tesseract cannot read the pair key.")
    args = parser.parse_args()

    USE_PADDLE_OCR = not args.no_paddle
    ...
    if args.no_paddle:
        log("PaddleOCR fallback disabled by --no-paddle; unread pair keys will be left for review.")
