...
DEFAULT_OUTPUT_DIR = Path("results") / "processing" / "2.groupped-bead-count"
DEFAULT_BATCH_OUTPUT_DIR = DEFAULT_OUTPUT_DIR
...
def build_final_payload(debug_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for row in debug_payload.get("images", []):
        ...
        pair_key = row.get("expectedPairKey")
        expected = pair_key if isinstance(pair_key, str) and re.fullmatch(r"\d+_\d+", pair_key) else ""
